...
public sealed class ScrollRectPool : MonoBehaviour, LoopScrollPrefabSource, LoopScrollDataSource, IPointerDownHandler, IPointerUpHandler
{
    [SerializeField]
    ...
    GameObject LoopScrollPrefabSource.GetObject(int index)
    {
        ...
        if (cell == null)
        {
            ...
            //cell.MusicElement.OnPointerDownEvent += OnPointerDown;
            //cell.MusicElement.OnPointerUpEvent += OnPointerUp;
        }
    ...
    public bool IsUserInteracting()
    {
        ...
        Debug.Log(LastScrollTime);
        if (Time.unscaledTime - LastScrollTime < SlipTime) return true;
        return false;
    }

    // イベントハンドラー
    public void OnPointerDown(PointerEventData eventData) { IsPointerDown = true; Debug.Log("Down"); }
    public void OnPointerUp(PointerEventData eventData) { IsPointerDown = false; Debug.Log("Up"); }
