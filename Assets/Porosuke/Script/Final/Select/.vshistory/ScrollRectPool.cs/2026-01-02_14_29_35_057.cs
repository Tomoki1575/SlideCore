...
public sealed class ScrollRectPool : MonoBehaviour, LoopScrollPrefabSource, LoopScrollDataSource, IPointerDownHandler, IPointerUpHandler, IScrollHandler
{
    ...
    public void SetupElements(List<MusicData> musicDataList, MusicDataManager.Difficulty difficulty, Color levelColor)
    {
        ...
        {
            ScrollRectComp.RefreshCells();
            // 以前の中央データのindexを探す
            if (MusicDataList.Contains(LastCenterMusic))
            {
                foreach(CellView cell in VisibleCells)
                {
                    if (cell.MusicData == LastCenterMusic) centerVirtualIndex = cell.Index;
                }
            }
        }
        // 即時再描画
        CalcAndSnapCenterElement(centerVirtualIndex, true);
    }

    private void OnElementPressed(CellView cell)
    {
        if (cell == null) return;
        CalcAndSnapCenterElement(cell.Index);
    }
    ...
    void LoopScrollDataSource.ProvideData(Transform trans, int index)
    {
        ...
        // データをセットする
        cell.MusicElement.SetElementData(MusicDataList[dataIndex], Difficulty, LevelColor, index);
    ...
    private void CalcAndSnapCenterElement(int virtualIndexToCenter = int.MaxValue, bool bInstant = false)
    {
        if (MusicDataList.Count == 0) return;
        WasInteractingLastFrame = false;

        int centerIndex = 0;
        if(virtualIndexToCenter != int.MaxValue)
        {
            centerIndex = virtualIndexToCenter;
        }
        else
        {
        ...
        if (bInstant) ScrollRectComp.ScrollToCellWithinTime(centerIndex, 0.01f, 0, LoopScrollRectBase.ScrollMode.ToCenter);
        else ScrollRectComp.ScrollToCell(centerIndex, 1000, 0, LoopScrollRectBase.ScrollMode.ToCenter);
    }
