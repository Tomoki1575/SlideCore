...
public sealed class ScrollRectPool : MonoBehaviour, LoopScrollPrefabSource, LoopScrollDataSource
{
    ...
    public CellView PreviousCell {  get; private set; }
    ...
    void LoopScrollDataSource.ProvideData(Transform trans, int index)
    {
        ...
        cell.MusicData = MusicDataList[dataIndex];
    }
    ...
    void LateUpdate()
    {
        ...
        if(PreviousCell) Debug.Log(PreviousCell.MusicData.title);
    }

    private void CalcAndSnapCenterElement(CellView center = null)
    {
        if (MusicDataList.Count == 0) return;
