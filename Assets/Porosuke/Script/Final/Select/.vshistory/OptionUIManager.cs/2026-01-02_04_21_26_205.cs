...
public class OptionUIManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        OnBaseNoteSpeedChanged(0, true);
        OnStartOffsetChanged(0, true);
        OnJudgeOffsetChanged(0, true);
        OnBGMSliderChanged((int)(OptionData.BGMVolume * 100));
        OnSESliderChanged((int)(OptionData.SEVolume * 100));
    }

    private void OnBaseNoteSpeedChanged(float value, bool bIgnore = false)
    {
        float speed = Mathf.Clamp(OptionData.UserNoteSpeed + value, BaseNoteSpeedMin, BaseNoteSpeedMax);
        if(bIgnore || OptionData.UserNoteSpeed != speed)
        {
    ...
    private void OnStartOffsetChanged(int value, bool bIgnore = false)
    {
        int offset = Math.Clamp(OptionData.NoteStartOffset + value, StartOffsetMin, StartOffsetMax);
        if(bIgnore || OptionData.NoteStartOffset != offset)
        {
    ...
    private void OnJudgeOffsetChanged(float value, bool bIgnore = false)
    {
        float offset = Mathf.Clamp(OptionData.JudgeOffset + value, JudgeOffsetMin, JudgeOffsetMax);
        if(bIgnore || OptionData.JudgeOffset != offset)
        {
