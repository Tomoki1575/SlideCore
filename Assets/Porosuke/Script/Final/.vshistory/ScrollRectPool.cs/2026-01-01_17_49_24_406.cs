...
public sealed class ScrollRectPool : MonoBehaviour, LoopScrollPrefabSource, LoopScrollDataSource
{
    ...
    void LateUpdate()
    {
        ...
        foreach (var item in VisibleElements)
        {
            ...
            // item.GetComponent<LayoutElement>().preferredHeight = item == centerItem ? 300 : 200;
            item.GetChild(0).GetComponent<TextMeshProUGUI>().text = item == centerItem ? $"***{index}***" : index.ToString();
            //if (index == centerIndex) le.preferredHeight = 300;
            //else if (index == centerIndex - 1) le.preferredHeight = 150;
            //else le.preferredHeight = 200;
        }

        // Debug.Log($"これが中央>{rectToIndex[centerItem]}");
        // スナップ
        ScrollRectComp.RefillCells(rectToIndex[centerItem] - 2);
    }
