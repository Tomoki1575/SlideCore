...
public sealed class ScrollRectPool : MonoBehaviour, LoopScrollPrefabSource, LoopScrollDataSource
{
    ...
    void LoopScrollDataSource.ProvideData(Transform trans, int index)
    {
        ...
        trans.GetChild(0).GetComponent<TextMeshProUGUI>().text = $"Virtual {index}";
    ...
    void LateUpdate()
    {
        ...
        foreach (var item in VisibleElements)
        {
            ...
            if (dist < minDist)
            {
                centerGap = itemCenter.y - viewportCenter.y;
                minDist = dist;
                centerItem = item;
            }
        }
        ...
        int expandIndex = centerIndex + CalcExpandElementIndexAdjustValue(centerGap);
        ...
        Debug.Log($"これが中央>{centerIndex}で、これを一番上にします。>{centerIndex - 3}");
        // スナップ
        ScrollRectComp.RefillCells(rectToIndex[centerItem] - 3);
    }

    private int CalcExpandElementIndexAdjustValue(float gap)
    {
        int size = 135;
        ...
        {
            value = (int)Mathf.Floor((gap - 1) / size) - 1;
        }
        else // gap < -n
        {
            value = -((int)Mathf.Floor(((-gap - 1) / size)) + 1);
