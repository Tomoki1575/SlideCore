...
public sealed class ScrollRectPool : MonoBehaviour, LoopScrollPrefabSource, LoopScrollDataSource
{
    ...
    void SnapToCenter(RectTransform centerItem)
    {
        // ScrollRect の運動を完全停止
        ...
        float normalizedDelta = deltaY / scrollableHeight;

        float current = ScrollRectComp.verticalNormalizedPosition;
        float target = Mathf.Clamp01(current + normalizedDelta);

        // ★ これが重要
        ScrollRectComp.verticalNormalizedPosition = target;
    }
