...
public class GameUIManager : MonoBehaviour
{
    ...
    public void SetConstantUI()
    {
        ...
        SetText(DefaultMusicWaitText, $"(DefaultMusicWaitSec : {OptionData.MusicStartWaitTime})");
        SetSliderValue(MusicWaitSlider, GameDataManager.MusicStartWaitSec, 0, 5);
        SetText(MusicWaitText, $"MusicWaitSec : {ConvertValueToString(GameDataManager.MusicStartWaitSec, 3)}");
        SetSliderValue(GameWaitSlider, GameDataManager.GameWaitSec, 0, 5);
        SetText(GameWaitText, $"GameWaitSec : {ConvertValueToString(GameDataManager.GameWaitSec, 3)}");
        SetSliderValue(OffsetSlider, GameDataManager.OffsetSec, 0, 5);
        SetText(OffsetText, $"OffsetSec : {ConvertValueToString(GameDataManager.OffsetSec, 3)}");
        SetText(BaseNoteSpeedText, $" × BaseNoteSpeed : {ConvertValueToString(OptionData.BaseNoteSpeed, 3)}");
        SetRectHeight(TotalLaneLengthRect, OptionData.LaneLength + OptionData.LaneAfterLength);
        SetSliderValue(NoteStartOffsetCoverSlider, OptionData.NoteStartOffset, 0, 100);
        SetText(NoteStartOffsetText, $"NoteStartOffset : {OptionData.NoteStartOffset}");
        SetRectHeight(LaneLengthRect, OptionData.LaneLength);
        SetText(LaneLengthText, OptionData.LaneLength.ToString());
        SetRectHeight(LaneAfterLengthRect, OptionData.LaneAfterLength);
        SetText(LaneAfterLengthText, OptionData.LaneAfterLength.ToString());
    }

    public void SetModifyUI(double virtualTime, bool bStartPlay, bool bPause, GameDataManager.NoteData firstNoteData)
    {
        // 常に変化するUIをセットする
        ...
        SetText(IsPauseText, $"IsPause : {bPause}");
        GameDataManager.SpeedSection section = GameDataManager.GetSectionAtTime(virtualTime);
        ...
        SetText(RemainNoteText, $"RemainNoteCount : {GameDataManager.NoteListListSize}");
        if(firstNoteData != null)
        {
            ...
            SetText(DestoryTimeText, $"Destory : {ConvertValueToString(firstNoteData.destroyTime, 3)}");
        }
    }
