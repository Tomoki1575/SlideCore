...
public class GameManager : MonoBehaviour
{
    ...
    private void ControlMusic()
    {
        ...
        if(VirtualTime < MusicStartWaitSec || VirtualTime >= (MusicStartWaitSec + SoundManagerClass.GetBGMLength()))
        {
            VirtualTime += Time.deltaTime;

            // 曲が停止し、全ノーツが破棄されたら終了
            if(NoteQueueSize == 0 && ActiveNoteListSize == 0)
            {
                Debug.Log("Finish!!");
                bStartPlay = false;
            }
    ...
    private void MoveNotes()
    {
        ...
        for (int i = ActiveNoteListSize - 1; i >= 0; i--)
        {
            ...
            if (VirtualTime < note.noteData.destroyTime)
            {
                ...
                // Hold線の描画
                LineManagerClass.AddHoldLine(note.noteData, position, VirtualTime);
