...
public class GameManager : MonoBehaviour
{
    ...
    {
        // もう生成すべきノーツがないなら何もしない
        if (NoteListSize == 0) return;
        // 未生成の最初のノーツがまだ現在時刻に達していないなら、その後ろもすべてまだ生成すべきではない（昇順前提）
        if (VirtualTime < NoteList[0].generateTime) return;

        // 未生成のノーツを前から走査する
        for (int i = 0; i < NoteListSize;)
        {
            NoteData noteData = NoteList[i];
            // 生成すべきノーツを見つけた（現在時刻が生成時刻と同じか、超えている）
            ...
            // ifの条件式を満たさなくなった（=このフレームにおける生成すべきノーツは全て生成し終えた）場合はすぐに終了
            else break;
        }
    }
