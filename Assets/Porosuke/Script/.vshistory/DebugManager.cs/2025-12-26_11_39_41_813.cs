...
public class DebugManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (NoteDataManagerClass) NonGenerateNoteDataList = NoteDataManagerClass.NoteList;
        ...
        SetText(MusicWaitText, $"MusicWait: {GetTotalOffsetSec()}");
    }
    ...
    {
        // 待機時間中はdeltaTimeで進める
        if(VirtualTime < GetTotalOffsetSec())
        {
        ...
        else
        {
            if (MusicSource)
            {
                ...
                {
                    MusicSource.time = 0;
                    VirtualTime = GetTotalOffsetSec() + MusicSource.time;
                    MusicSource.Play();
                }
                else VirtualTime = GetTotalOffsetSec() + MusicSource.time;
            }
        }
    }
